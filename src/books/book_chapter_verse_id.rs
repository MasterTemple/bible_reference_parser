...
use super::book_segment::BookSegment;
...
// pub struct BookChapterVerse<'a>(BookSegment<'a, ChapterVerse>);
pub struct BookChapterVerse {
    book: u8,
    chapter: u8,
    verse: u8,
...
pub const VERSES_BEFORE_BOOK: [u16; 66] = [
    0, 1533, 2746, 3605, 4893, 5852, 6510, 7128, 7213, 8023, 8718, 9534, 10253, 11195, 12017,
    12297, 12703, 12870, 13940, 16401, 17316, 17538, 17655, 18947, 20311, 20465, 21738, 22095,
    22292, 22365, 22511, 22532, 22580, 22685, 22732, 22788, 22841, 22879, 23090, 23145, 24216,
    24894, 26045, 26924, 27931, 28364, 28801, 29058, 29207, 29362, 29466, 29561, 29650, 29697,
    29810, 29893, 29939, 29964, 30267, 30375, 30480, 30541, 30646, 30659, 30673, 30698,
];
...
impl BookChapterVerse {
    fn book_idx(&self) -> usize {
        (self.book - 1) as usize
    }
    fn chapter_idx(&self) -> usize {
        (self.chapter - 1) as usize
    }
    ...
    pub fn is_valid(book: u8, chapter: u8, verse: u8) -> bool {
        Self::new(book, chapter, verse).is_ok()
    }
    ...
    pub fn new(book: u8, chapter: u8, verse: u8) -> Result<Self, String> {
        ...
        let verse_err = || {
            format!(
                "There is no 'Verse {}' in 'Chapter {}' of 'Book {}'",
                verse, chapter, book
            )
        };
        ...
        let chapter_verse_counts = BOOK_CHAPTER_VERSE_COUNT
            .get(book_idx)
            .ok_or_else(book_err)?;
        let chapter_verse_count = chapter_verse_counts
            .get(chapter_idx)
            .ok_or_else(chapter_err)?;
        if verse > (*chapter_verse_count as u8) {
            Err(verse_err())?
        }
    ...
    pub fn from_verse(mut id: u16) -> Result<Self, String> {
        ...
        id -= 1;
        let verses_before_book_idx = VERSES_BEFORE_BOOK.binary_search(&id);
        let book_idx = match verses_before_book_idx {
            Ok(idx) => idx,
            Err(idx) => {
                if idx >= VERSES_BEFORE_BOOK.len() {
                    return Err(format!("There is not 'Book {}' in the Bible", idx));
                }
                idx - 1
            }
        };
        let verses_before_book = VERSES_BEFORE_BOOK[book_idx];

        let chapter_verse_counts = BOOK_CHAPTER_VERSE_COUNT[book_idx];
        let mut remaining = id - verses_before_book;

        let mut chapter = 1;
        let mut iter = chapter_verse_counts.iter();
    ...
    pub fn as_verse(&self) -> u16 {
        let verses_before_book = VERSES_BEFORE_BOOK[self.book_idx()];

        let book_chapters = BOOK_CHAPTER_VERSE_COUNT[self.book_idx()];
        let verses_before_chapter: u16 = book_chapters.iter().take(self.chapter_idx()).sum();
    ...
    pub fn from_id_string(input: &str) -> Result<BookChapterVerse, String> {
        if input.len() != 8 {
            return Err(format!("Expected length of 8: 2 digits for the book, 3 digits for the chapter, and 3 digits for the verse"));
        }
        let book = &input[0..=1];
        let book = book
            .parse()
            .map_err(|_| format!("Could not parse book from '{}'", book))?;
        let chapter = &input[2..=4];
        let chapter = chapter
            .parse()
            .map_err(|_| format!("Could not parse chapter from '{}'", chapter))?;
        let verse = &input[5..=7];
        let verse = verse
            .parse()
            .map_err(|_| format!("Could not parse verse from '{}'", verse))?;
        BookChapterVerse::new(book, chapter, verse)
    }
...
mod book_chapter_verse_tests {
    ...
    fn as_verse() -> Result<(), String> {
        assert_eq!(BookChapterVerse::new(1, 1, 1)?.as_verse(), 1,);

        assert_eq!(BookChapterVerse::new(1, 1, 2)?.as_verse(), 2,);

        assert_eq!(BookChapterVerse::new(40, 1, 1)?.as_verse(), 23146,);

        assert_eq!(BookChapterVerse::new(40, 1, 25)?.as_verse(), 23170,);

        assert_eq!(BookChapterVerse::new(40, 2, 1)?.as_verse(), 23171,);
    ...
    fn remaining_verses() -> Result<(), String> {
        ...
        assert_eq!(BookChapterVerse::new(1, 1, 31)?.remaining_verses(), None);
    ...
    fn iter_remaining_verses() -> Result<(), String> {
        assert_eq!(
            BookChapterVerse::new(1, 1, 1)?
                .iter_remaining_verses()
                .collect_vec(),
            (2..=31).collect_vec()
        );

        assert_eq!(
            BookChapterVerse::new(1, 1, 30)?
                .iter_remaining_verses()
                .collect_vec(),
            (31..=31).collect_vec()
        );
    ...
    fn remaining_chapters() -> Result<(), String> {
        ...
        assert_eq!(BookChapterVerse::new(1, 50, 1)?.remaining_chapters(), None);
    ...
    fn iter_remaining_chapters() -> Result<(), String> {
        assert_eq!(
            BookChapterVerse::new(1, 1, 1)?
                .iter_remaining_chapters()
                .collect_vec(),
            (2..=50).collect_vec()
        );

        assert_eq!(
            BookChapterVerse::new(1, 49, 1)?
                .iter_remaining_chapters()
                .collect_vec(),
            (50..=50).collect_vec()
        );
